export const site = {
  title: "Joseph Dubon",
  siteUrl: "https://josephdubon.github.io",
  description:
    "Personal site for Joseph Dubon, a web developer and photographer helping small businesses and personal brands look more professional online.",
  location: "Reno, Nevada",
  role: "Websites and photography that help small businesses look professional and get more clients.",
  intro:
    "I build websites and create photography that help small businesses and personal brands present themselves clearly online. Better visuals, better structure, and a stronger first impression.",
  email: "josephdubon@protonmail.com",
  phone: "775-232-6280",
  availability:
    "Available for website projects, photography sessions, and combined visual + web work.",
  ogImage: "/images/joseph-dubon-og.svg",
  github: "https://github.com/josephdubon",
  currentEdition: {
    label: "Spring 2026",
    title: "Helping people look more professional online",
    summary:
      "I combine web development, visual presentation, and photography so the site and the imagery support the same story instead of working against each other.",
    stats: [
      { label: "Experience", value: "10+ years" },
      { label: "Web focus", value: "Custom sites" },
      { label: "Photo focus", value: "Branding + portraits" },
      { label: "Clients", value: "Small business + personal brands" },
    ],
  },
  socialLinks: [
    {
      label: "GitHub",
      href: "https://github.com/josephdubon",
    },
    {
      label: "Email",
      href: "mailto:josephdubon@protonmail.com",
    },
    {
      label: "Phone",
      href: "tel:7752326280",
    },
  ],
};

export const nowItems = [
  "Custom websites and landing pages for small businesses and independent brands.",
  "Portrait, branding, and creative photography for a stronger visual presence.",
  "Combined web and photography work so the site and the imagery feel intentional together.",
];

export const principles = [
  {
    title: "Web development",
    text: "Custom websites, small business sites, landing pages, and updates that keep the experience clear and usable.",
  },
  {
    title: "Photography",
    text: "Portrait, branding, and creative photography that gives people better visuals for websites, profiles, and marketing.",
  },
  {
    title: "Combined presence",
    text: "When the site and the imagery are built together, the final result feels more credible, more consistent, and more professional.",
  },
];

export const projects = [
  {
    name: "Client Delivery and Technical Consulting",
    type: "Digital delivery foundation",
    year: "2026",
    description:
      "A long-running base of client delivery work across websites, applications, integrations, and technical problem solving that shaped how I scope, ship, and support systems.",
    tags: ["Delivery", "Consulting", "Systems Thinking"],
  },
  {
    name: "Internal Tools and AI Workflows",
    type: "AI-enabled systems",
    year: "2026",
    description:
      "Built internal tooling and AI-assisted workflow support to improve communication, reduce manual coordination, and make day-to-day execution more reliable.",
    tags: ["Automation", "Internal Tools", "AI Workflows"],
  },
  {
    name: "Technical Operations Leadership",
    type: "Operations and management",
    year: "Ongoing",
    description:
      "Led 32 staff across 11 sites while building reporting systems, improving accountability, and standardizing execution across a complex operating environment.",
    tags: ["Operations", "Leadership", "Execution"],
  },
];
